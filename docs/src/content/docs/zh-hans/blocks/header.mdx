...
name: "标头"
heading: "标头组件"
description: "仅 CSS Tailwind CSS 标题块，带有导航栏、品牌和移动抽屉。粘性、透明和响应式布局变体。 WCAG AA 可访问，可与 Django、HTMX、Laravel、React 和任何堆栈配合使用。"
metaTitle: "标题 |仅 CSS Tailwind CSS 块 |Frutjam"
metaDescription: "纯 CSS Tailwind CSS 标头块具有粘性、响应式和品牌布局。没有 JavaScript，可访问 WCAG AA。适用于 Django、HTMX、Laravel、React 和任何堆栈。"
order: 0
related:
  - "components/navbar"
  - "components/drawer"
...
使用适用于移动设备的 [header 组件](/components/header) 、[navbar](/components/navbar) 和 [drawer](/components/drawer) 构建的现成标头布局。
