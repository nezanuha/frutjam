...
name: "テーブル"
heading: "テーブルコンポーネント"
description: "CSS のみの Tailwind CSS テーブル。ゼブラストライプ、コンパクトなデフォルトサイズ、ピン留めされた列、および行ホバー状態を備えています。 WCAG AA にアクセス可能で、Django、HTMX、Laravel、React、およびあらゆるスタックで動作します。"
metaTitle: "表 - CSS のみの Tailwind CSS コンポーネント |Frutjam"
metaDescription: "Tailwind CSS テーブルはストライプ行、スティッキーヘッダー、コンパクトなサイズ設定を備えています。ダッシュボードやリストでアクセスしやすく応答性の高いデータを表示するためのセマンティック HTML。"
order: 0
related:
  - "components/checkbox"
  - "components/button"
...
テーブル コンポーネントは、構造化データを行と列に編成して表示します。セマンティック HTML テーブル要素と Tailwind CSS スタイルで構築されたテーブルは、並べ替え、ストライプ行、ホバー効果、および応答性の高いオーバーフロー処理をサポートします。 Frutjam テーブル システムは、ヘッダーを適切に関連付けることでアクセスでき、データ視覚化フレームワークとシームレスに連携します。
...
| クラス | タイプ | 説明 |
| --- | --- | --- |
| table | <span class="badge badge-xs">ベース</span> | Styled HTML table with consistent spacing |
| table-zebra | <span class="badge badge-xs badge-success badge-soft">スタイル</span> | Alternating row background colors |
| table-zebra-rows | <span class="badge badge-xs badge-success badge-soft">スタイル</span> | Alternating row background on tbody rows only |
| table-zebra-cols | <span class="badge badge-xs badge-success badge-soft">スタイル</span> | Alternating column background colors |
| table-hover | <span class="badge badge-xs badge-success badge-soft">スタイル</span> | Highlights rows on hover |
| table-pin-rows | <span class="badge badge-xs badge-secondary badge-soft">修飾子</span> | Sticks header and footer rows while scrolling |
| table-pin-cols | <span class="badge badge-xs badge-secondary badge-soft">修飾子</span> | Sticks first and last columns while scrolling |
| table-xs | <span class="badge badge-xs badge-info badge-soft">サイズ</span> | Extra small row padding |
| table-sm | <span class="badge badge-xs badge-info badge-soft">サイズ</span> | Small row padding |
| table-md | <span class="badge badge-xs badge-info badge-soft">サイズ</span> | Medium row padding (default) |
| table-lg | <span class="badge badge-xs badge-info badge-soft">サイズ</span> | Large row padding |
| table-xl | <span class="badge badge-xs badge-info badge-soft">サイズ</span> | Extra large row padding |
