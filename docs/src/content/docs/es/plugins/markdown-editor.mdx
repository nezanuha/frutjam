...
La mayoría de los editores de rebajas de JavaScript (EasyMDE, SimpleMDE, editores basados ​​en CodeMirror) ** ocultan su `<textarea>`** y edite una copia, escribiendo el contenido cuando se envíe el formulario. Eso funciona hasta que algo más necesita el valor: un `required` El campo en el que el navegador no puede centrarse bloquea el envío por completo, y `.value` o `FormData` leer antes de enviar devuelve una cadena vacía, lo que rompe las protecciones de htmx, Turbo, autoguardado y cambios no guardados. **MarkdownEditor es diferente**: aplica estilo al área de texto que ya tienes y la deja como el campo en el que escribes, de modo que el valor sea correcto en todo momento.
...
| --- | --- | --- |
| Área de texto nativa preservada | ✅ | ❌ Oculto, editado como copia |
| Funciona con `required` campos | ✅ | ❌ El navegador bloquea el envío |
| `.value` corregir antes de enviar | ✅ | ❌ Vacío hasta que se envíe el formulario |
| Se serializa con `FormData`, htmx, Turbo | ✅ | ❌ Necesita la API propia del editor |
| Modo híbrido WYSIWYG | ✅ | ❌ |
| Buscar y reemplazar incorporado | ✅ | ❌ |
| soporte RTL | ✅ Construido en | A través de CodeMirror `direction` opción |
| Controladores de eventos en línea (CSP) | ✅ Ninguno | Alguno |
| Modo oscuro/temática | ✅ | Limitado |
| Tamaño del paquete, comprimido | 51KB | 107KB (JS+CSS) |
