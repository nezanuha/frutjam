...
name: "英雄"
heading: "英雄组件"
description: "仅限 CSS Tailwind CSS 登陆页面英雄部分。全角布局，标题、描述和 CTA 居中。 WCAG AA 可访问，可与 Django、HTMX、Laravel、React 和任何堆栈配合使用。"
metaTitle: "Hero - 纯 CSS Tailwind CSS 组件 |Frutjam"
metaDescription: "Tailwind CSS Hero 部分具有居中、分割和叠加布局。带有 CTA 按钮和媒体支持的语义 HTML，可实现高转化率的登陆页面。"
order: 0
related:
  - "blocks/hero"
  - "components/button"
...
英雄组件创建引人注目的全角部分，通过引人注目的视觉效果和消息传递来介绍您的页面或产品。英雄部分采用语义 HTML 和灵活的布局构建，为用户参与定下了基调。 Frutjam 英雄系统支持多种背景、叠加和响应式对齐，非常适合登陆页面、产品发布和重要公告。
...
| 班级 | 类型 | 描述 |
| --- | --- | --- |
| hero | <span class="badge badge-xs">根据</span> | Full-width section with centered content |
| hero-content | <span class="badge badge-xs badge-secondary badge-soft">修饰符</span> | Inner content area with flex layout |
| hero-overlay | <span class="badge badge-xs badge-secondary badge-soft">修饰符</span> | Semi-transparent overlay for background images |
