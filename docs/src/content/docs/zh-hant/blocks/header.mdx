...
name: "標頭"
heading: "標頭組件"
description: "僅 CSS Tailwind CSS 標題塊，帶有導航列、品牌和移動抽屜。黏性、透明和響應式佈局變體。 WCAG AA 可訪問，可與 Django、HTMX、Laravel、React 和任何堆疊配合使用。"
metaTitle: "標題 |僅 CSS Tailwind CSS 塊 |Frutjam"
metaDescription: "純 CSS Tailwind CSS 標頭區塊具有黏性、響應式和品牌佈局。沒有 JavaScript，可存取 WCAG AA。適用於 Django、HTMX、Laravel、React 和任何堆疊。"
order: 0
related:
  - "components/navbar"
  - "components/drawer"
...
使用適用於行動裝置的 [header 組件](/components/header) 、[navbar](/components/navbar) 和 [drawer](/components/drawer) 建構的現成標頭佈局。
